...
-- Hero Section
CREATE TABLE church_hero_sections (
...
CREATE TABLE church_about_sections (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
-- Team Members
CREATE TABLE church_team_members (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  role TEXT NOT NULL,
...
-- Website Settings
CREATE TABLE church_website_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
-- Create indexes
CREATE INDEX idx_church_hero_organization ON church_hero_sections(organization_id);
CREATE INDEX idx_church_about_organization ON church_about_sections(organization_id);
CREATE INDEX idx_church_team_organization ON church_team_members(organization_id);
CREATE INDEX idx_church_events_organization ON church_events(organization_id);
CREATE INDEX idx_church_events_date ON church_events(date);
CREATE INDEX idx_church_schedules_organization ON church_schedules(organization_id);
CREATE INDEX idx_church_contact_organization ON church_contact_info(organization_id);
CREATE INDEX idx_prayer_requests_organization ON prayer_requests(organization_id);
CREATE INDEX idx_church_sermons_organization ON church_sermons(organization_id);
CREATE INDEX idx_church_sermons_date ON church_sermons(date);
CREATE INDEX idx_church_website_settings_organization ON church_website_settings(organization_id);
...
CREATE POLICY "Organization members can modify hero sections" ON church_hero_sections
  FOR ALL USING (organization_id = user_organization_id());

CREATE POLICY "Organization members can modify about sections" ON church_about_sections
  FOR ALL USING (organization_id = user_organization_id());

CREATE POLICY "Organization members can modify team members" ON church_team_members
  FOR ALL USING (organization_id = user_organization_id());

CREATE POLICY "Organization members can modify events" ON church_events
  FOR ALL USING (organization_id = user_organization_id());

CREATE POLICY "Organization members can modify schedules" ON church_schedules
  FOR ALL USING (organization_id = user_organization_id());

CREATE POLICY "Organization members can modify contact info" ON church_contact_info
  FOR ALL USING (organization_id = user_organization_id());

CREATE POLICY "Organization members can modify sermons" ON church_sermons
  FOR ALL USING (organization_id = user_organization_id());

CREATE POLICY "Organization members can modify website settings" ON church_website_settings
  FOR ALL USING (organization_id = user_organization_id());
...
CREATE POLICY "Organization members can read prayer requests" ON prayer_requests
